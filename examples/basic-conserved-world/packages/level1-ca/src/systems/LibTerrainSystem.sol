...
import { CHUNK_MIN_Y, TILE_Y, Biome, STRUCTURE_CHUNK, STRUCTURE_CHUNK_CENTER, AirVoxelID, GrassVoxelID, DirtVoxelID, BedrockVoxelID } from "../Constants.sol";
...
contract LibTerrainSystem is System {
  ...
  function getHeight(int32 x, int32 z, int128[4] memory biome) internal view returns (int32) {
    ...
    int128 perlin999 = IWorld(_world()).ca_PerlinSystem_noise2d(x - 550, z + 550, 999, 64);
    int128 continentalHeight = continentalness(perlin999);
    int128 terrainHeight = Math.mul(perlin999, _10);
    int128 perlin49 = IWorld(_world()).ca_PerlinSystem_noise2d(x, z, 49, 64);
    terrainHeight = Math.add(terrainHeight, Math.mul(perlin49, _5));
    terrainHeight = Math.add(terrainHeight, IWorld(_world()).ca_PerlinSystem_noise2d(x, z, 13, 64));
    ...
    int128 valley = valleys(
      Math.div(Math.add(Math.mul(IWorld(_world()).ca_PerlinSystem_noise2d(x, z, 333, 64), _2), perlin49), _3)
    );
  ...
  function getBiome(int32 x, int32 z) internal view returns (int128[4] memory) {
    int128 heat = IWorld(_world()).ca_PerlinSystem_noise2d(x + 222, z + 222, 444, 64);
    int128 humidity = IWorld(_world()).ca_PerlinSystem_noise(z, x, 999, 333, 64);
  ...
  function applySpline(int128 x, Tuple[] memory splines) internal view returns (int128) {
    ...
    return IWorld(_world()).ca_PerlinSystem_lerp(t, points[0].y, points[1].y);
  }
