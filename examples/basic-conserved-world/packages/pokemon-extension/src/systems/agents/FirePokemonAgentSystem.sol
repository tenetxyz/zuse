...
import { CA_ADDRESS, REGISTRY_ADDRESS, FirePokemonVoxelID } from "@tenet-pokemon-extension/src/Constants.sol";
...
contract FirePokemonAgentSystem is AgentType {
  function registerBody() public override {
    ...
    pokemonChildVoxelTypes[0] = FirePokemonVoxelID;
    bytes32 baseVoxelTypeId = FirePokemonVoxelID;
    ...
    registerVoxelType(
      ...
      VoxelSelectors({
        enterWorldSelector: IWorld(world).pokemon_FirePokemonAgent_enterWorld.selector,
        exitWorldSelector: IWorld(world).pokemon_FirePokemonAgent_exitWorld.selector,
        voxelVariantSelector: IWorld(world).pokemon_FirePokemonAgent_variantSelector.selector,
        activateSelector: IWorld(world).pokemon_FirePokemonAgent_activate.selector,
  ...
  function getInteractionSelectors() public override returns (InteractionSelector[] memory) {
    ...
    voxelInteractionSelectors[0] = InteractionSelector({
      interactionSelector: IWorld(_world()).pokemon_FirePokemonAgent_replenishEnergyEventHandler.selector,
