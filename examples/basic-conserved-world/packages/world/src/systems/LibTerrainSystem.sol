...
uint256 constant Y_AIR_THRESHOLD = 100;
uint256 constant Y_GROUND_THRESHOLD = 0;
...
contract LibTerrainSystem is System {
  ...
  function getTerrainProperties(VoxelCoord memory coord) public view returns (uint256, uint256) {
    ...
    if (isGround || isTerrain) {
      ...
      mass = uint256((massNoise * int128(TOTAL_MASS_IN_CHUNK)) / (denom * 2));
      ...
      energy = uint256((energyNoise * int128(TOTAL_ENERGY_IN_CHUNK)) / (denom * 2)); // Normalize and scale
    }
