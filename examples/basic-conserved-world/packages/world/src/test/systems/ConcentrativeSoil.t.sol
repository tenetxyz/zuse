...
import { ConcentrativeSoilVoxelID, ProteinSoilVoxelID, ElixirSoilVoxelID, PlantVoxelID } from "@tenet-pokemon-extension/src/Constants.sol";
...
contract ConcentrativeSoilTest is MudTest {
  ...
  function testConcentrativeSoilWithSoilNeighbour() public returns (VoxelEntity memory, VoxelEntity memory) {
    ...
    VoxelEntity memory soilEntity2 = world.buildWithAgent(ConcentrativeSoilVoxelID, soilCoord2, agentEntity, bytes4(0));
  ...
  function testConcentrativeSoilWithPlantNeighbour() public returns (VoxelEntity memory, VoxelEntity memory) {
    vm.startPrank(alice, alice);
    VoxelEntity memory agentEntity = setupAgent();
  ...
  function testConcentrativeSoilBehaviour() public returns (VoxelEntity memory, VoxelEntity memory) {
    vm.startPrank(alice, alice);
    VoxelEntity memory agentEntity = setupAgent();
    ...
    vm.roll(block.number + 1);
    console.log("activate");
    world.activateWithAgent(ConcentrativeSoilVoxelID, soilCoord2, agentEntity, bytes4(0));
