...
contract ElixirSoilTest is MudTest {
  ...
  function testElixirSoilWithSoilNeighbour() public returns (VoxelEntity memory, VoxelEntity memory) {
  ...
  function testElixirSoilWithPlantNeighbour() public returns (VoxelEntity memory, VoxelEntity memory) {
    vm.startPrank(alice, alice);
    VoxelEntity memory agentEntity = setupAgent();
    ...
    uint256 plantElixir = Elixir.get(IStore(SIMULATOR_ADDRESS), worldAddress, plantEntity.scale, plantEntity.entityId);
  ...
  function testElixirSoilWithSoilAndPlantNeighbour() public {
    ...
    assertTrue(soil2Nutrients == 0);
    uint256 soil2N = Nitrogen.get(IStore(SIMULATOR_ADDRESS), worldAddress, soilEntity2.scale, soilEntity2.entityId);
    assertTrue(soil2N > 0);
    assertTrue(Phosphorous.get(IStore(SIMULATOR_ADDRESS), worldAddress, soilEntity2.scale, soilEntity2.entityId) == 0);
    assertTrue(Potassium.get(IStore(SIMULATOR_ADDRESS), worldAddress, soilEntity2.scale, soilEntity2.entityId) == 0);
    ...
    assertTrue(soil2Nutrients == 0);
    assertTrue(Nitrogen.get(IStore(SIMULATOR_ADDRESS), worldAddress, soilEntity2.scale, soilEntity2.entityId) > soil2N);
