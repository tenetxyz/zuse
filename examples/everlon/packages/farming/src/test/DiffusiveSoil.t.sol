...
contract DiffusiveSoilTest is MudTest {
  ...
  function testDiffusiveSoilWithSoilNeighbour() public {
    ...
    bytes32 soilEntityId = world.build(agentObjectEntityId, DiffusiveSoilObjectID, soilCoord);
    ...
    world.activate(agentObjectEntityId, DiffusiveSoilObjectID, soilCoord);
    ...
    // Diffusive soil should not have transferred nutrients to soil 2
    assertTrue(soil2Nutrients == 0, "Soil 2 nutrients not 0");

    vm.stopPrank();
  }

  function testDiffusiveSoilWithPlantNeighbour() public {
    vm.startPrank(alice, alice);
    (, bytes32 agentObjectEntityId) = setupAgent();
  ...
  function testDiffusiveSoilBehaviour() public {
    vm.startPrank(alice, alice);
    (, bytes32 agentObjectEntityId) = setupAgent();
    ...
    uint256 soil2BeforeNutrients = soil1Nutrients + 50;
    Nutrients.set(simStore, worldAddress, soil2ObjectEntityId, soil2BeforeNutrients);

    vm.roll(block.number + 1);

    world.activate(agentObjectEntityId, DiffusiveSoilObjectID, soilCoord2);
    // Diffusive soil should have gotten nutrients to soil 2
    assertTrue(
