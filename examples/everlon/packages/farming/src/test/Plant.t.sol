...
contract PlantTest is MudTest {
  ...
  function testPlant() public {
    ...
    assertTrue(produced > 0, "Produced not > 0");
    console.log("plantData");
    console.logUint(Elixir.get(simStore, worldAddress, plantObjectEntityId));
    console.logUint(Protein.get(simStore, worldAddress, plantObjectEntityId));
    console.logAddress(worldAddress);
    console.logBytes32(plantObjectEntityId);
