...
contract ProteinSoilTest is MudTest {
  ...
  function testProteinSoilWithSoilNeighbour() public {
    ...
    bytes32 soilEntityId = world.build(agentObjectEntityId, ProteinSoilObjectID, soilCoord);
    ...
    world.activate(agentObjectEntityId, ProteinSoilObjectID, soilCoord);
    ...
    // Protein soil should not have transferred nutrients to soil 2
    assertTrue(soil2Nutrients == 0, "Soil 2 nutrients not 0");

    vm.stopPrank();
  }

  function testProteinSoilWithPlantNeighbour() public {
    vm.startPrank(alice, alice);
    (, bytes32 agentObjectEntityId) = setupAgent();
  ...
  function testProteinSoilWithSoilAndPlantNeighbour() public {
    ...
    assertTrue(soil2P > 0, "Phosphorus not found for soil 2");
    assertTrue(Potassium.get(simStore, worldAddress, soil2ObjectEntityId) > 0, "Potassium not found for soil 2");
    ...
    Energy.set(simStore, worldAddress, soilObjectEntityId, 150);
    // Protein soil should transfer phosphorus out
    Phosphorus.set(simStore, worldAddress, soilObjectEntityId, soil2P + 10);
    world.activate(agentObjectEntityId, ProteinSoilObjectID, soilCoord);
    ...
    assertTrue(Phosphorus.get(simStore, worldAddress, soilObjectEntityId) == 0, "Phosphorus not 0");
    assertTrue(
      Phosphorus.get(simStore, worldAddress, soil2ObjectEntityId) > soil2P,
      "Phosphorus not greater than before"
    );
