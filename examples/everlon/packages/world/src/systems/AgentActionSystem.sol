...
contract AgentActionSystem is System {
  function hit(bytes32 agentObjectEntityId, bytes32 targetObjectEntityId, uint32 damage) public {
    require(hasKey(OwnedByTableId, OwnedBy.encodeKeyTuple(agentObjectEntityId)), "MindSystem: entity has no owner");
    require(OwnedBy.get(agentObjectEntityId) == _msgSender(), "MindSystem: caller does not own entity");
