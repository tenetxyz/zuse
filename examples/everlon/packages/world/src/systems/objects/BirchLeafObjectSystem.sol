...
import { REGISTRY_ADDRESS, NUM_BLOCK_STACKABLE, BirchLeafObjectID, BIRCH_LEAF_MASS } from "@tenet-world/src/Constants.sol";

contract BirchLeafObjectSystem is ObjectType {
  function registerObject() public {
    address world = _world();
    IObjectRegistrySystem(REGISTRY_ADDRESS).registerObjectType(
      BirchLeafObjectID,
      world,
      IWorld(world).world_BirchLeafObjectS_enterWorld.selector,
      IWorld(world).world_BirchLeafObjectS_exitWorld.selector,
  ...
  function enterWorld(
    ...
    objectProperties.mass = BIRCH_LEAF_MASS;
