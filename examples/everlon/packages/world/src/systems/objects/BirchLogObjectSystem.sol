...
import { REGISTRY_ADDRESS, NUM_BLOCK_STACKABLE, BirchLogObjectID, BIRCH_LOG_MASS } from "@tenet-world/src/Constants.sol";

contract BirchLogObjectSystem is ObjectType {
  function registerObject() public {
    address world = _world();
    IObjectRegistrySystem(REGISTRY_ADDRESS).registerObjectType(
      BirchLogObjectID,
      world,
      IWorld(world).world_BirchLogObjectSy_enterWorld.selector,
      IWorld(world).world_BirchLogObjectSy_exitWorld.selector,
      IWorld(world).world_BirchLogObjectSy_eventHandler.selector,
  ...
  function enterWorld(
    ...
    objectProperties.mass = BIRCH_LOG_MASS;
