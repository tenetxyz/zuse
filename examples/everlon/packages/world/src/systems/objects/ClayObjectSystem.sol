...
import { REGISTRY_ADDRESS, NUM_BLOCK_STACKABLE, ClayObjectID, CLAY_MASS } from "@tenet-world/src/Constants.sol";

contract ClayObjectSystem is ObjectType {
  function registerObject() public {
    address world = _world();
    IObjectRegistrySystem(REGISTRY_ADDRESS).registerObjectType(
      ClayObjectID,
      world,
      IWorld(world).world_ClayObjectSystem_enterWorld.selector,
      IWorld(world).world_ClayObjectSystem_exitWorld.selector,
  ...
  function enterWorld(
    ...
    objectProperties.mass = CLAY_MASS;
