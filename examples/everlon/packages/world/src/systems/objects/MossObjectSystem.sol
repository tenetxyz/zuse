...
import { REGISTRY_ADDRESS, NUM_BLOCK_STACKABLE, MossObjectID, MOSS_MASS } from "@tenet-world/src/Constants.sol";

contract MossObjectSystem is ObjectType {
  function registerObject() public {
    address world = _world();
    IObjectRegistrySystem(REGISTRY_ADDRESS).registerObjectType(
      MossObjectID,
      world,
      IWorld(world).world_MossObjectSystem_enterWorld.selector,
      IWorld(world).world_MossObjectSystem_exitWorld.selector,
  ...
  function enterWorld(
    ...
    objectProperties.mass = MOSS_MASS;
