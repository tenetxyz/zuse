...
import { REGISTRY_ADDRESS, NUM_BLOCK_STACKABLE, SoilObjectID, SOIL_MASS } from "@tenet-world/src/Constants.sol";

contract SoilObjectSystem is ObjectType {
  function registerObject() public {
    address world = _world();
    IObjectRegistrySystem(REGISTRY_ADDRESS).registerObjectType(
      SoilObjectID,
      world,
      IWorld(world).world_SoilObjectSystem_enterWorld.selector,
      IWorld(world).world_SoilObjectSystem_exitWorld.selector,
