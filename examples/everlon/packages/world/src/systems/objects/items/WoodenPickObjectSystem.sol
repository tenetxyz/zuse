...
import { REGISTRY_ADDRESS, NUM_BLOCK_STACKABLE, WoodenPickObjectID, WOODEN_PICK_MASS } from "@tenet-world/src/Constants.sol";

contract WoodenPickObjectSystem is ObjectType {
  function registerObject() public {
    address world = _world();
    IObjectRegistrySystem(REGISTRY_ADDRESS).registerObjectType(
      WoodenPickObjectID,
      world,
      IWorld(world).world_WoodenPickObject_enterWorld.selector,
      IWorld(world).world_WoodenPickObject_exitWorld.selector,
      IWorld(world).world_WoodenPickObject_eventHandler.selector,
  ...
  function enterWorld(
    ...
    objectProperties.mass = WOODEN_PICK_MASS;
