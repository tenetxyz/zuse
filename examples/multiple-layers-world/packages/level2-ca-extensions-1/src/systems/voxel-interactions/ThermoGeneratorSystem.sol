...
import { Generator, TemperatureAtTime, GeneratorData, Temperature, TemperatureData, TemperatureAtTimeData } from "@tenet-level2-ca-extensions-1/src/codegen/Tables.sol";
import { BlockDirection } from "@tenet-utils/src/Types.sol";
import { entityIsGenerator, entityHasTemperature } from "@tenet-level2-ca-extensions-1/src/InteractionUtils.sol";
...
contract ThermoGeneratorSystem is VoxelInteraction {
  ...
  function neighbourEventHandlerThermoGenerator(
    address callerAddress,
    bytes32 neighbourEntityId,
    bytes32 centerEntityId
  ) public returns (bool, bytes memory) {
