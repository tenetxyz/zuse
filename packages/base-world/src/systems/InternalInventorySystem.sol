...
abstract contract InternalInventorySystem is System {
  ...
  function isInventoryFull(bytes32 objectEntityId) public view virtual returns (bool);
  ...
  function addObjectToInventory(
    ...
    require(stackable > 0, "InternalInventorySystem: Object type is not stackable");
    ...
    while (remainingObjectsToAdd > 0) {
      ...
      require(inventoryIds.length < NUM_MAX_INVENTORY_SLOTS, "InternalInventorySystem: Inventory is full");
  ...
  function removeObjectFromInventory(bytes32 inventoryId, uint8 numObjectsToRemove) public virtual {
    InventoryObjectData memory inventoryObjectData = InventoryObject.get(inventoryId);
    require(inventoryObjectData.numObjects >= numObjectsToRemove, "InternalInventorySystem: Not enough objects");
