...
import { VoxelCoord, Quaternion } from "../types.sol";
import { hasKey } from "@latticexyz/world/src/modules/keysintable/hasKey.sol";
import { PlayerPosition, PlayerDirection, PlayerDirectionData, PlayerPositionTableId, Position, PlayerPositionData, PositionTableId, VoxelType, VoxelTypeData, VoxelTypeRegistry } from "../codegen/Tables.sol";
...
contract MoveSystem is System {
  ...
  function rotate(Quaternion memory newQuaternion) public {
    bytes32 playerEntity = addressToEntityKey(_msgSender());
    // check if player has position
    // bytes32[] memory keyTuple = new bytes32[](1);
    // keyTuple[0] = bytes32((playerEntity));
    ...
    // TODO: Add delta logic to check if new position is within bounds
    PlayerDirectionData memory newDirection = PlayerDirectionData({
      qx: newQuaternion.qx,
      qy: newQuaternion.qy,
      qz: newQuaternion.qz,
      qw: newQuaternion.qw
    ...
    PlayerDirection.set(playerEntity, newDirection);
  }
}
