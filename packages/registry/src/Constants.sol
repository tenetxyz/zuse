// SPDX-License-Identifier: MIT
pragma solidity >=0.8.0;

string constant REGISTER_BODY_TYPE_SIG = "registerBodyType(string,bytes32,bytes32,bytes32[],bytes32[],bytes32,(bytes4,bytes4,bytes4,bytes4,bytes4,(bytes4,string,string)[]))";
string constant REGISTER_BODY_VARIANT_SIG = "registerBodyVariant(bytes32,(uint256,uint32,bool,bool,bool,uint8,bytes,string))";
string constant REGISTER_CA_SIG = "registerCA(string,string,bytes32[])";
string constant ADD_BODY_CA_SIG = "addBodyToCA(bytes32)";
string constant REGISTER_WORLD_SIG = "registerWorld(string,string,address[])";
string constant WORLD_NOTIFY_NEW_CA_BODY_TYPE_SIG = "onNewCABodyType(address,bytes32)";
string constant REGISTER_CREATION_SIG = "registerCreation(string,string,(bytes32,bytes32)[],(int32,int32,int32)[],(bytes32,(int32,int32,int32),(int32,int32,int32)[])[])";
string constant GET_VOXELS_IN_CREATION_SIG = "getVoxelsInCreation(bytes32)";
string constant CREATION_SPAWNED_SIG = "creationSpawned(bytes32)";
string constant BODY_SPAWNED_SIG = "bodySpawned(bytes32)";
string constant REGISTER_MIND_SIG = "registerMind(bytes32,(address,string,string,bytes4))";
string constant REGISTER_MIND_WORLD_SIG = "registerMindForWorld(bytes32,address,(address,string,string,bytes4))";
